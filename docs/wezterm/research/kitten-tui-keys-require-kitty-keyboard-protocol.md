...
- [wezterm.lua](../../../modules/shared/gui/wezterm/config/wezterm.lua) を確認 → `enable_kitty_keyboard` の指定なし(= デフォルト `false` が適用されていた)。
...
- **WezTermのこのプロトコル実装には既知のバグ報告がある。** 有効化は無条件に安全ではない:
  ...
  - このため [wezterm.lua](../../../modules/shared/gui/wezterm/config/wezterm.lua) には**警告コメント付き**で有効化した。今後、他のTUIアプリ(nvim等)でキー入力に違和感が出たら、まずこのオプションを疑って一時的に無効化して切り分けること。
...
- [wezterm.lua](../../../modules/shared/gui/wezterm/config/wezterm.lua) — 実際に修正した設定ファイル(警告コメント付き)
